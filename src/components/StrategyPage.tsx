...
export function StrategyPage() {
  return (
    <main className="bg-white p-2">
      <section className="relative overflow-hidden rounded-2xl bg-background pb-24 pt-36 max-[991px]:pb-16 max-[479px]:pt-28">
        <div className="site-container px-6 max-[479px]:px-4">
          ...
          <div className="grid items-start gap-14 lg:grid-cols-[280px_minmax(0,1fr)]">
            <aside className="sticky top-28 hidden rounded-2xl border border-primary/10 bg-white/70 p-4 backdrop-blur lg:block">
    ...
    nodes.push(
      ...
      >
        {listItems.map((text, index) => (
          <li
            key={`${text}-${index}`}
            className={`relative text-lg leading-[150%] before:absolute before:-left-5 before:top-[0.7rem] before:size-2 before:rounded-full before:bg-accent max-[479px]:text-base ${
  ...
  items.forEach((item, index) => {
    ...
    if (item.type === "table") {
      nodes.push(
        <div
          key={`${item.type}-${index}`}
          className="my-8 overflow-hidden rounded-2xl border border-primary/12 bg-white"
        >
